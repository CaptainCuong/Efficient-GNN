...
class Calib_Random(BaseAttack):
    ...
    def attack(self, ori_features, ori_adj, target_node, n_perturbations, strategy='under',
        ...
        **kwargs : dict
        ...
        original_output = self.surrogate(features_tensor, adj_tensor).detach()[[target_node]]
        ...
        elif torch.is_tensor(ori_adj):
            modified_adj = sp.lil_matrix(ori_adj.cpu().numpy())
        else:
            modified_adj = sp.lil_matrix(ori_adj)

        if sp.issparse(ori_features):
            modified_features = ori_features.tolil()
        elif torch.is_tensor(ori_features):
            modified_features = sp.lil_matrix(ori_features.cpu().numpy())
        else:
        ...
        for perturbation_step in range(n_perturbations):
            ...
            for trial in range(max_trials):
                ...
                if perturb_structure:
                    # Random edge perturbation focused on target node
                    success, perturbation_info = self._random_target_edge_perturbation(
                        trial_adj, target_node
                    )
                else:
                    # Random feature perturbation on target node
                    success, perturbation_info = self._random_target_feature_perturbation(
                ...
                trial_adj_tensor, trial_features_tensor = self._prepare_inputs(trial_adj, trial_features)

                with torch.no_grad():
                    new_output = self.surrogate(trial_features_tensor, trial_adj_tensor)[[target_node]]
        ...
        if torch.is_tensor(ori_adj):
            self.modified_adj = torch.FloatTensor(best_adj.todense()).to(self.device)
        else:
            self.modified_adj = sp.csr_matrix(best_adj)

        if torch.is_tensor(ori_features):
            self.modified_features = torch.FloatTensor(best_features.todense()).to(self.device)
        else:
