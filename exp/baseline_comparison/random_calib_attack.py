...
def run_random_attacks(attack_model, calibrated_model, features, adj, labels, target_nodes,
    ...
    for strategy in strategies:
        ...
        attack_times = [r['attack_time'] for r in strategy_results]
        iteration_times = [r['iteration_time'] for r in strategy_results]
...
def compare_attack_strategies(attack_results):
    ...
    for strategy, results in attack_results.items():
        ...
        if successful:
            ...
            avg_attack_time = np.mean([r['attack_time'] for r in results])
            avg_iteration_time = np.mean([r['iteration_time'] for r in results])
        else:
            avg_conf_change = 0
            avg_perturbations = 0
            std_conf_change = 0
        ...
        print(f"  Average attack time: {avg_attack_time:.4f}s")
        print(f"  Average iteration time: {avg_iteration_time:.4f}s")


def parse_arguments():
    """Parse command line arguments for random attack configuration."""
    parser = argparse.ArgumentParser(description='Random Attack for Calibrated GNNs')
    parser.add_argument('--attack-nodes', type=int, default=100,
                        help='Number of nodes to attack (default: 100)')
    parser.add_argument('--budget', type=int, default=5,
                        help='Number of perturbations allowed per node (default: 5)')
    parser.add_argument('--strategies', nargs='+', default=['under', 'over', 'under_kl'],
                        choices=['under', 'over', 'under_kl'],
                        help='Attack strategies to use (default: under over under_kl)')
...
def main():
    ...
    # Select test nodes for attack (same selection logic as ugca_calib_attack.py)
    ...
    print(f"Attack configuration: {args.budget} perturbations per node, strategies: {args.strategies}")
