#!/usr/bin/env python3
"""
FGA (Feature Gradient Attack) for GNN Calibration

This script demonstrates the Feature Gradient Attack (FGA) on calibrated GNNs:
1. Training a base GNN model
2. Calibrating it with various calibration methods (CaGCN, ETS, GETS, MS, TS, VS, WATS, SimCalib)
3. Attacking the calibrated model using FGA, which perturbs node features based on
   gradient information to reduce prediction confidence while preserving predicted labels
"""
...
def save_attack_log(args, attack_results, base_metrics, calibrated_metrics, experiment_config, log_dir="./logs"):
    """Save comprehensive FGA attack results and experiment information to log files."""
    ...
    log_data = {
        "experiment_info": {
            "timestamp": timestamp,
            "attack_type": "FGA (Feature Gradient Attack)",
        ...
        "attack_summary": {
            ...
            "timing": {
                "total_time": float(np.sum([r['iteration_time'] for r in attack_results])),
                "avg_attack_time": float(np.mean([r['attack_time'] for r in attack_results])),
                "avg_iteration_time": float(np.mean([r['iteration_time'] for r in attack_results])),
                "min_iteration_time": float(np.min([r['iteration_time'] for r in attack_results])),
                "max_iteration_time": float(np.max([r['iteration_time'] for r in attack_results]))
            }
        },
    ...
    with open(summary_file, 'w') as f:
        f.write("=" * 80 + "\n")
        f.write("FGA (Feature Gradient Attack) EXPERIMENT SUMMARY\n")
        f.write("=" * 80 + "\n")
        f.write(f"Timestamp: {timestamp}\n")
        f.write(f"Attack Type: FGA (Feature Gradient Attack)\n")
...
def run_attack_on_nodes(attack_model, calibrated_model, features, adj, labels, target_nodes, budget=5):
    """
    Run FGA (Feature Gradient Attack) on specified nodes.

    FGA perturbs node features based on gradient information to reduce prediction
    confidence while preserving the predicted label (underconfidence attack).
    """
    print(f"\nRunning FGA (Feature Gradient Attack) on {len(target_nodes)} nodes with budget {budget}...")
    ...
    for i, node in enumerate(target_nodes):
        ...
        # Get original prediction and confidence before FGA attack
        with torch.no_grad():
        ...
        # Run FGA attack: uses feature gradients to perturb node features
        attack_start_time = time.time()
        attack = Calib_FGA(attack_model, device=features.device)

        n_perturb = []  # Track number of feature perturbations
        best_conf = []  # Track best (lowest) confidence achieved
        ...
        attack.attack(
            ...
            strategy="under",  # Underconfidence attack: reduce confidence while preserving label
            res_gt=labels,
            verbose=False,
            beam_width=3,  # Beam search width for exploring perturbations
            n_perturb=n_perturb,
            best_conf=best_conf
        )
        attack_time = time.time() - attack_start_time

        # Evaluate model after FGA attack (with perturbed features)
        with torch.no_grad():
        ...
        if original_pred != attacked_pred:
            raise ValueError("FGA attack failed: label changed (should preserve predicted label).")

        iteration_time = time.time() - iteration_start_time

        # Store FGA attack results
        result = {
            'node': int(node),
        ...
        print(f"  After FGA: pred={attacked_pred}, conf={attacked_conf:.4f}")
        print(f"  Confidence change: {result['conf_change']:+.4f}, Feature perturbations: {result['perturbations']}")
    ...
    print(f"\nFGA Attack Summary:")
    print(f"Successful FGA attacks (label preserved + confidence reduced): {successful_attacks}/{len(target_nodes)}")
    print(f"\nTiming Statistics:")
...
def parse_arguments():
    """Parse command line arguments for FGA attack configuration."""
    parser = argparse.ArgumentParser(description='FGA (Feature Gradient Attack) for Calibrated GNNs')
    ...
    parser.add_argument('--budget', type=int, default=5,
                        help='Number of feature perturbations allowed per node (default: 5)')

    # FGA attack configuration arguments
    parser.add_argument('--attack-nodes', type=int, default=100,
                        help='Number of nodes to attack with FGA (default: 100)')
...
def main():
    ...
    # Step 3: Launch FGA attack on the calibrated model
    print("\n" + "="*50)
    print("LAUNCHING FGA (FEATURE GRADIENT ATTACK) ON CALIBRATED MODEL")
    print("="*50)

    # Select test nodes for FGA attack
    ...
    print(f"Selected {num_attack_nodes} nodes for FGA attack (out of {len(available_test_nodes)} test nodes)")
    print(f"FGA configuration: {budget} feature perturbations per node, beam width = 3")

    # Experiment configuration for logging
    experiment_config = {
        "target_nodes": len(test_node_indices),
        "budget": budget,
        "attack_strategy": "underconfidence",  # FGA targets underconfidence
        "attack_mechanism": "feature_gradient_based",  # FGA uses feature gradients
        "beam_width": 3,
    ...
    attack_results = run_attack_on_nodes(
        attack_model=calibrated_model,  # Use calibrated model for computing feature gradients
    ...
    # Analyze FGA attack results
    print("\n" + "="*50)
    print("FGA ATTACK ANALYSIS")
    print("="*50)
    ...
    print(f"FGA success rate: {len(successful_attacks)}/{len(attack_results)} ({len(successful_attacks)/len(attack_results)*100:.1f}%)")
    print(f"Average confidence reduction: {avg_conf_reduction:.4f}")
    print(f"Average feature perturbations used: {avg_perturbations:.1f}")
    print(f"\nTiming Statistics:")
    print(f"  Total time: {total_time:.2f}s")
    print(f"  Average attack time per node: {avg_attack_time:.4f}s")
    print(f"  Average iteration time per node: {avg_iteration_time:.4f}s")

    # Show some example FGA results
    print("\nExample FGA attack results:")
    for result in attack_results[:5]:
        status = "SUCCESS" if result['label_preserved'] and result['conf_change'] < 0 else "FAILED"
        print(f"Node {result['node']}: {result['original_conf']:.4f} → {result['attacked_conf']:.4f} "
              f"({result['conf_change']:+.4f}) [{status}]")

    # Save comprehensive FGA logs
    print("\n" + "="*50)
    print("SAVING FGA ATTACK LOGS")
    print("="*50)
