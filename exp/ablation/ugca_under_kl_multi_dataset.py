#!/usr/bin/env python3
"""
UGCA (Underconfidence Gradient Calibration Attack) with KL Divergence for Multiple Datasets

This script demonstrates the UGCA attack using the basic attack function with "under_kl" strategy
(KL divergence with uniform distribution) on calibrated GNNs across multiple datasets:
1. Training a base GNN model
2. Calibrating it with various calibration methods (CaGCN, ETS, GETS, MS, TS, VS, WATS, SimCalib)
3. Attacking the calibrated model using attack with strategy="under_kl" from Calib_FGA
...
def save_attack_log(args, attack_results, base_metrics, calibrated_metrics, attacked_metrics, experiment_config, log_dir="./logs"):
    ...
    log_data = {
        "experiment_info": {
            "timestamp": timestamp,
            "attack_type": "UGCA (attack with under_kl strategy)",
    ...
    # Save JSON log
    json_file = os.path.join(log_dir, f"ugca_under_kl_{args.dataset.lower()}_{args.calibration_method.lower()}_attack_log_{timestamp}.json")
    with open(json_file, 'w') as f:
        json.dump(log_data, f, indent=2)

    # Save human-readable summary
    summary_file = os.path.join(log_dir, f"ugca_under_kl_{args.dataset.lower()}_{args.calibration_method.lower()}_attack_summary_{timestamp}.txt")
    with open(summary_file, 'w') as f:
        f.write("=" * 80 + "\n")
        f.write("UGCA UNDER_KL ATTACK EXPERIMENT SUMMARY\n")
        f.write("=" * 80 + "\n")
        f.write(f"Timestamp: {timestamp}\n")
        f.write(f"Attack Type: UGCA (attack with under_kl strategy - KL divergence with uniform)\n")
...
def run_attack_on_nodes(attack_model, calibrated_model, features, adj, labels, target_nodes, budget=5):
    """Run attack with under_kl strategy on specified nodes."""
    print(f"\nRunning UGCA (attack with under_kl strategy) on {len(target_nodes)} nodes with budget {budget}...")
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='UGCA Under_KL Attack for Multiple Datasets')
    ...
    parser.add_argument('--attack-nodes', type=int, default=1000,
                        help='Number of nodes to attack (default: 1000)')
...
def main():
    ...
    print("\n" + "="*50)
    print("ATTACKING CALIBRATED MODEL WITH UGCA (UNDER_KL STRATEGY)")
    print("="*50)
    ...
    experiment_config = {
        ...
        "budget": budget,
        "attack_strategy": "under_kl",
        "attack_method": "attack",
        "loss_function": "kl_divergence_with_uniform",
