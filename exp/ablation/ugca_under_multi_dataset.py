...
def save_attack_log(args, attack_results, base_metrics, calibrated_metrics, attacked_metrics, experiment_config, log_dir="./logs"):
    ...
    # Save JSON log
    json_file = os.path.join(log_dir, f"ugca_under_{args.dataset.lower()}_attack_log_{timestamp}.json")
    with open(json_file, 'w') as f:
        json.dump(log_data, f, indent=2)

    # Save human-readable summary
    summary_file = os.path.join(log_dir, f"ugca_under_{args.dataset.lower()}_attack_summary_{timestamp}.txt")
