#!/usr/bin/env python3
"""
UGCA (Underconfidence Gradient Calibration Attack) with Rerank for Multiple Datasets

This script demonstrates the UGCA attack using rerank_hybridloss_attack on calibrated GNNs
across multiple datasets:
1. Training a base GNN model
2. Calibrating it with various calibration methods (CaGCN, ETS, GETS, MS, TS, VS, WATS, SimCalib)
3. Attacking the calibrated model using rerank_hybridloss_attack from Calib_FGA
...
def save_attack_log(args, attack_results, base_metrics, calibrated_metrics, attacked_metrics, experiment_config, log_dir="./logs"):
    ...
    log_data = {
        "experiment_info": {
            "timestamp": timestamp,
            "attack_type": "UGCA (rerank_hybridloss_attack)",
    ...
    # Save JSON log
    json_file = os.path.join(log_dir, f"ugca_rerank_hybridloss_{args.dataset.lower()}_{args.calibration_method.lower()}_attack_log_{timestamp}.json")
    with open(json_file, 'w') as f:
        json.dump(log_data, f, indent=2)

    # Save human-readable summary
    summary_file = os.path.join(log_dir, f"ugca_rerank_hybridloss_{args.dataset.lower()}_{args.calibration_method.lower()}_attack_summary_{timestamp}.txt")
    with open(summary_file, 'w') as f:
        f.write("=" * 80 + "\n")
        f.write("UGCA RERANK ATTACK EXPERIMENT SUMMARY\n")
        f.write("=" * 80 + "\n")
        f.write(f"Timestamp: {timestamp}\n")
        f.write(f"Attack Type: UGCA (rerank_hybridloss_attack)\n")
...
def run_attack_on_nodes(attack_model, calibrated_model, features, adj, labels, target_nodes, budget=5):
    """Run rerank_hybridloss_attack on specified nodes."""
    print(f"\nRunning UGCA (rerank_hybridloss_attack) on {len(target_nodes)} nodes with budget {budget}...")
    ...
    for i, node in enumerate(target_nodes):
        ...
        attack.rerank_hybridloss_attack(
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='UGCA Rerank Attack for Multiple Datasets')
...
def main():
    ...
    print("\n" + "="*50)
    print("ATTACKING CALIBRATED MODEL WITH UGCA (RERANK_HYBRIDLOSS_ATTACK)")
    print("="*50)
    ...
    experiment_config = {
        ...
        "attack_method": "rerank_hybridloss_attack",
