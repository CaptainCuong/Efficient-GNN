#!/usr/bin/env python3
"""
UGCA (Underconfidence Gradient Calibration Attack) with Basic Rerank for Multiple Datasets

This script demonstrates the UGCA attack using rerank_attack on calibrated GNNs
across multiple datasets:
1. Training a base GNN model
2. Calibrating it with various calibration methods (CaGCN, ETS, GETS, MS, TS, VS, WATS, SimCalib)
3. Attacking the calibrated model using rerank_attack from Calib_FGA
...
def save_attack_log(args, attack_results, base_metrics, calibrated_metrics, attacked_metrics, experiment_config, log_dir="./logs"):
    ...
    log_data = {
        "experiment_info": {
            "timestamp": timestamp,
            "attack_type": "UGCA (rerank_attack)",
    ...
    with open(summary_file, 'w') as f:
        ...
        f.write(f"Attack Type: UGCA (rerank_attack)\n")
...
def run_attack_on_nodes(attack_model, calibrated_model, features, adj, labels, target_nodes, budget=5):
    """Run rerank_attack on specified nodes."""
    print(f"\nRunning UGCA (rerank_attack) on {len(target_nodes)} nodes with budget {budget}...")
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='UGCA Rerank Basic Attack for Multiple Datasets')
...
def main():
    ...
    print("\n" + "="*50)
    print("ATTACKING CALIBRATED MODEL WITH UGCA (RERANK_ATTACK)")
    print("="*50)
