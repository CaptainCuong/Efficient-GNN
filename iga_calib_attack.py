#!/usr/bin/env python3
"""
Temperature Scaling Calibration followed by Calib_IGA Attack

This script demonstrates:
1. Training a base GNN model
2. Calibrating it with Temperature Scaling (TS)
3. Attacking the calibrated model using Calib_IGA (Integrated Gradients Attack)
"""
...
def run_calib_iga_attacks(attack_model, calibrated_model, features, adj, labels, target_nodes,
    ...
    """Run Calib_IGA attacks with different strategies on specified nodes."""
    print(f"\nRunning Calib_IGA attacks on {len(target_nodes)} nodes...")
...
def compare_attack_strategies(attack_results):
    ...
    print("\n" + "="*60)
    print("CALIB_IGA ATTACK STRATEGY COMPARISON")
    print("="*60)
...
def main():
    ...
    # Step 3: Attack the calibrated model with Calib_IGA
    print("\n" + "="*50)
    print("ATTACKING CALIBRATED MODEL WITH CALIB_IGA")
    print("="*50)

    # Select test nodes for attack (first 20 test nodes)
    test_node_indices = torch.nonzero(data.test_mask).flatten()[:20].cpu().numpy()

    # Run attacks with different strategies
    attack_results = run_calib_iga_attacks(
    ...
    analyze_integrated_gradients_effectiveness(attack_results)
    ...
    print(f"\nCalib_IGA attack characteristics:")
    ...
    print(f"- Calib_IGA provides more principled edge selection than random attacks")
    print(f"- More computationally expensive than Calib_FGA but potentially more effective")
